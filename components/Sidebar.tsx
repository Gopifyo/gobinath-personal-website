...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, onNavigate, onReset }) => {
  ...
  return (
    <>
      {/* Overlay for mobile */}
      <div 
        className={`fixed inset-0 bg-black/50 backdrop-blur-md z-30 md:hidden transition-opacity duration-300 ${
      ...
      `}>
        ...
        <div className="p-5">
          ...
          >
            <Plus size={18} className="text-zinc-400 group-hover:text-cyan-400" />
            New Session
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 scrollbar-none">
          <div className="mb-4 space-y-1">
            ...
            <NavButton icon={<ImageIcon size={18} />} label="Can I see the visual vault?" onClick={() => onNavigate('Gallery')} />
...
const NavButton = ({ icon, label, onClick }: { icon: any, label: string, onClick: () => void }) => (
  ...
  >
    <span className="text-zinc-600 group-hover:text-cyan-400 transition-colors group-hover:scale-110 duration-300 shrink-0">{icon}</span>
